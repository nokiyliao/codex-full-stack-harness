...
describe("GatewayClient", () => {
  test("reads the canonical control deck from its fixed read-only endpoint", async () => {
    let observedUrl = "";
    const client = new GatewayClient({
      ...
      fetch: async (input) => {
        ...
        return jsonResponse({
          ...
          state_head: "a".repeat(64),
          session_db_state_head: "b".repeat(64),
          convergence_state_head: "c".repeat(64),
          observed_at_ms: 1,
    ...
    const snapshot = await client.controlDeck();

    expect(observedUrl).toBe("http://gateway.test/control-deck");
    expect(snapshot.authority_effect).toBe("none");
  });
